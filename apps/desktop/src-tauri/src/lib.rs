...
fn create_rack_project(
    ...
    if final_root.exists() {
        return Err(format!("{} already exists. Choose another Rack name or location.", final_root.display()));
    }

    let staging = parent.join(format!(".rack-create-{}-{}", folder_name, std::process::id()));
    if staging.exists() {
...
fn write_generated_file(path: String, content: String) -> Result<(), String> {
    ...
    if destination.exists() {
        ...
        if metadata.file_type().is_symlink() || !metadata.is_file() {
            return Err("Rack will only replace an ordinary file chosen through the save dialog.".to_string());
        }
    }
    ...
    let temporary = parent.join(format!(".rack-output-{file_name}-{}.tmp", std::process::id()));
